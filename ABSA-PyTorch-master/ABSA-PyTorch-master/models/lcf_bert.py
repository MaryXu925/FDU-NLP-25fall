...
except ImportError:  # transformers<4 compatibility
...
class LCF_BERT(nn.Module):
    ...
    def feature_dynamic_mask(self, text_local_indices, aspect_indices):
        ...
        for text_i, asp_i in zip(range(len(texts)), range(len(asps))):
            ...
            for i in range(mask_begin):
                masked_text_raw_indices[text_i][i] = np.zeros((self.opt.bert_dim), dtype=np.float32)
            for j in range(asp_begin + asp_len + mask_len, self.opt.max_seq_len):
                masked_text_raw_indices[text_i][j] = np.zeros((self.opt.bert_dim), dtype=np.float32)
    ...
    def forward(self, inputs):
        ...
        attention_mask_global = text_bert_indices.ne(0).long()
        attention_mask_local = text_local_indices.ne(0).long()
        ...
        except TypeError:  # transformers<3
            bert_spc_out, _ = self.bert_spc(
                text_bert_indices,
                token_type_ids=bert_segments_ids,
                attention_mask=attention_mask_global,
        ...
        try:
            bert_local_out, _ = self.bert_local(
                text_local_indices,
